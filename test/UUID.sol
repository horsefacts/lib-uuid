...
contract UUIDTest is Test {
    ...
    function test_parseTooLong() public {
        vm.expectRevert(UUID.InvalidLength.selector);
        UUID.parse(TOO_LONG);
    }
    ...
    function test_parseMalformed1() public {
        vm.expectRevert(UUID.MalformedUUID.selector);
        UUID.parse(MALFORMED_1);
    }

    function test_parseMalformed2() public {
        vm.expectRevert(UUID.MalformedUUID.selector);
        UUID.parse(MALFORMED_2);
    }

    function test_parseMalformed3() public {
        vm.expectRevert(UUID.MalformedUUID.selector);
        UUID.parse(MALFORMED_3);
    }

    function test_parseMalformed4() public {
        vm.expectRevert(UUID.MalformedUUID.selector);
        UUID.parse(MALFORMED_4);
    }

    function test_parseInvalidByte1() public {
        vm.expectRevert(UUID.InvalidByte.selector);
        UUID.parse(INVALID_BYTE_1);
    }

    function test_parseInvalidByte2() public {
        vm.expectRevert(UUID.InvalidByte.selector);
        UUID.parse(INVALID_BYTE_2);
    }

    function test_parseInvalidByte3() public {
        vm.expectRevert(UUID.InvalidByte.selector);
        assertEq(UUID.parse(INVALID_BYTE_3), "");
    }

    function test_parseInvalidByte4() public {
        vm.expectRevert(UUID.InvalidByte.selector);
        UUID.parse(INVALID_BYTE_4);
    }

    function test_parseValid() public {
        assertEq(UUID.parse(VALID_1), bytes16(0x07ad2cedd8134b1496795389d38f7a5e));
        assertEq(UUID.parse(VALID_2), bytes16(0x1ab0f6a1a14148c4ae40cc2e2913d504));
        assertEq(UUID.parse(VALID_3), bytes16(0xe6851052e94b455badf03986cc0ede55));
        assertEq(UUID.parse(VALID_4), bytes16(0x32b97deba70649cbab41acdd546037a1));
    }

    function test_format() public {
        assertEq(UUID.toString(bytes16(0x07ad2cedd8134b1496795389d38f7a5e)), VALID_1);
        assertEq(UUID.toString(bytes16(0x1ab0f6a1a14148c4ae40cc2e2913d504)), VALID_2);
        assertEq(UUID.toString(bytes16(0xe6851052e94b455badf03986cc0ede55)), VALID_3);
